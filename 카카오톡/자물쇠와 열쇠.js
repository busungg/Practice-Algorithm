...
function checkAllKeys(key, lock, virtualKey) {
    const rotKeys = [], keyLen = key.length, lockLen = lock.length;
    let tempKey, originKey;

    for(let i = 0; i < 4; i++) { //0도 부터 시작 , i * 90도로 생각
        if(i === 0) {
            originKey = key;
        } else {
    ...
    //한칸은 걸치도록 한다.
    let start = -(keyLen - 1), end = lockLen, isTrue;
        ...
        for(let startRow = start; startRow < end; startRow++ ) {
            for(let startCol = start; startCol < end; startCol++ ) {
                ...
                for(let i = 0; i < keyLen; i++) {
                    for(let j = 0; j < keyLen; j++) {
                        if(startRow + i >= 0 && startCol + j >= 0 
                            && startRow + i < lockLen && startCol + j < lockLen 
                            && rotKeys[rot][i][j] === 1) {
                ...
                if(tempKey.length === virtualKey.length) {
                    for(let k = 0; k < tempKey.length; k++) {
                        if(tempKey[k][2] !== 1 || tempKey[k][0] !== virtualKey[k][0] || tempKey[k][1] !== virtualKey[k][1]){
                            isTrue = false;
                            break;
                        }
                    }
